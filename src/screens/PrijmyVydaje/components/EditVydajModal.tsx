/** EditVydajModal - Modální okno pro editaci výdajů na tabu Příjmy */
...
// Typy pro výdaje na tabu Příjmy
export interface VydajPrijmy {
  id: string;
  castka: number;
  datum: string;
...
export interface EditVydajModalProps {
  visible: boolean;
  vydaj: VydajPrijmy | null;
  onClose: () => void;
  onSave: (editedVydaj: VydajPrijmy) => Promise<void>;
  onDelete?: (vydaj: VydajPrijmy) => Promise<void>;
}

/**
 * @description Modální okno pro editaci výdajů s kategoriemi Zboží/Provoz
 */
export const EditVydajModal: React.FC<EditVydajModalProps> = ({
  ...
  const [kategorie, setKategorie] = useState<'ZBOZI' | 'PROVOZ'>('ZBOZI');
  ...
  const handleKategorieChange = (novaKategorie: 'ZBOZI' | 'PROVOZ') => {
  ...
  const handleSave = async () => {
    ...
    if (!dodavatel.trim()) {
      Alert.alert('Chyba', 'Zadejte název dodavatele');
      return;
    }
  ...
  const handleDelete = () => {
    ...
    Alert.alert(
      'Smazat výdaj',
      `Opravdu chcete smazat výdaj "${getKategorieNazev(vydaj.kategorie)}" za ${vydaj.castka.toLocaleString()} Kč?`,
      [
  ...
  const getKategorieNazev = (kat: 'ZBOZI' | 'PROVOZ') => {
    switch (kat) {
      case 'ZBOZI':
        return 'Zboží';
      case 'PROVOZ':
              ...
              <View style={styles.kategorieContainer}>
                <TouchableOpacity
                  style={[
                    ...
                    kategorie === 'ZBOZI' && styles.kategorieButtonSelected
                  ]}
                  onPress={() => handleKategorieChange('ZBOZI')}
                >
                  <View style={styles.kategorieButtonContent}>
                    <View style={[
                      styles.kategorieIndicator,
                      kategorie === 'ZBOZI' && styles.kategorieIndicatorSelected
                    ]} />
                    <Text style={[
                      styles.kategorieButtonText,
                      kategorie === 'ZBOZI' && styles.kategorieButtonTextSelected
                    ]}>
                      Zboží
                    </Text>
                  </View>
