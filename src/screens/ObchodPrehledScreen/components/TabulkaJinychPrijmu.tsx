...
interface TabulkaJinychPrijmuProps {
  ...
  scrollable?: boolean; // Nová prop pro kontrolu scrollování
}
...
export const TabulkaJinychPrijmu: React.FC<TabulkaJinychPrijmuProps> = ({
  ...
  formatujDatum,
  onSmazat,
  scrollable = true, // Výchozí hodnota true pro zpětnou kompatibilitu
}) => {
  ...
  return (
    <View style={styles.container}>
      ...
      {scrollable ? (
        // Použij FlatList když je scrollable = true (pro samostatnou obrazovku)
        <FlatList
          data={jinePrijmy}
        ...
        // Použij obyčejný View s mapováním když je scrollable = false (pro vnoření v ScrollView)
